...
def write_derivative_description(bids_dir, deriv_dir):
    ...
    desc = {
        'Name': 'atlasTransform - transforming 3D and 4D nifti files into atlas space',
        'BIDSVersion': '1.1.1',
        'PipelineDescription': {
            'Name': 'atlasTransform',
            'Version': __version__,
    ...
    if 'atlasTransform'.upper() + '_DOCKER_TAG' in os.environ:
        desc['DockerHubContainerTag'] = os.environ['atlasTransform'.upper() +'_DOCKER_TAG']
    if 'atlasTransform'.upper() + '_SINGULARITY_URL' in os.environ:
        singularity_url = os.environ['atlasTransform'.upper() + '_SINGULARITY_URL']
...
def collect_data(layout: BIDSLayout, subject_id, other_format: str = None):
    ...
    masks = [file.split('desc')[0] +'desc-brain_mask.nii.gz' for file in standard_search['bold']]
...
class BIDSPlusDataGrabberOutputSpec(BIDSDataGrabberOutputSpec):
    ...
    confounds = OutputMultiObject(desc='confounds tsv')
